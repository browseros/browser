...
export class StateHelper {
    ...
    public static changeStateByChangeTabUrl(state: fromApp.State, tabId: number, url: string): fromApp.State {
        ...
        let changedTab = state.tabs[changedTabIndex];
        let oldAppId = changedTab.appId;
        let newHost = this.extractHostname(url);
        let newAppId = state.host2Apps[newHost];
        ...
        return this.changeStateByMoveTabWithNewUrlToExistApp(state, tabId, url, newAppId, oldAppId);
    }
    ...
    public static prepareAppLink(app: string) {
        ...
        if (app.indexOf('.') < 0) {
            return `https://www.google.com/search?q=${app}`;
        }
        return `http://${app}`;
    }
    ...
    private static getNextCurrentTabIndexBeforeTabId(appId: number, tabs: ITab[], tabId: number): ITab | null {
        let appTabs = tabs.filter(t => t.appId === appId);
        let currentTabIndex = appTabs.findIndex(t => t.id === tabId);
        if (currentTabIndex < 0) {
            return null;
        }
        if (currentTabIndex === 0) {
            return appTabs.length > 1 ? appTabs[1] : null;
        }
        return appTabs[currentTabIndex - 1];
    }

    private static getNextCurrentAppBeforeAppId(appId: number, apps: IApp[]): IApp | null {
        let currentAppIndex = apps.findIndex(a => a.id === appId);
        if (currentAppIndex < 0) {
            return null;
        }
        if (currentAppIndex === 0) {
            return apps.length > 1 ? apps[1] : null;
        }
        return apps[currentAppIndex - 1];
    }
